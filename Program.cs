...
if ((i == a) && (b == c))
{
    ...
    Console.Write($" Так как первая цифра {i} совпадает с последней цифрой {a}, Вторая цифра {b} совпадает с предпоследней цифрой {c} ");
}
else
{
    Console.Write($" Число {n} не является палиндромом.");
    Console.Write($" Первая цифра {i}, пятая {a}, вторая {b} , четвертая {c} ");
}
